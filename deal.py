...
if not os.path.exists('./result/cleansample/'):
    os.makedirs('./result/cleansample/')
for file in file_name:
     with open("./result/bwasam/"+file+".sam","r") as pdinf:
        count={} 
        for i in range(1,1+len(ref)):
            count[str(i)]=0
        print ('Start processing file:',file)
        with open("./result/cleansample/"+file+".txt","w") as out:
            with open("./result/cleansample/"+file+"int.txt","w") as out1:
                for lines in pdinf.readlines()[2:]:
                    ...
                    if "M" in letraw:            
                        if len(letraw)==1 and let[0]=="M":
                            out.write(">"+line[0]+"\t"+line[5]+"\t"+line[3]+"\t"+line[9]+"\n")
                            ...
                            if "D" in letraw or "I" in letraw or "N" in letraw or "S" in letraw or "H" in letraw: 
                                out.write(">"+line[0]+"\t"+line[5]+"\t"+line[3]+"\t"+seqraw+"\n") 
                                # print (">"+line[0]+"\t"+line[5]+"\t"+line[3]+"\t"+seqraw+"\n")
...
datalast.to_csv('./result/datalast.txt',sep='\t',index=False)
